...
export default function ListView({ onCardClick }: { onCardClick: (id: number) => void }) {
          ...
          <div key={col.id} className="mb-6">
            ...
            {cards.map((card) => (
              <div key={card.id} onClick={() => onCardClick(card.id)}
                className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg mb-1 cursor-pointer transition-colors"
                style={{ background: "#161625", border: "1px solid #2a2a4a" }}
