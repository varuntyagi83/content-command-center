...
export default function KanbanBoard({ onCardClick }: { onCardClick: (id: number) => void }) {
  ...
  const onDragStart = (e: React.DragEvent, card: ContentCard) => {
    setDragCard(card);
  ...
  const onDrop = (e: React.DragEvent, colId: ColumnId) => {
    e.preventDefault();
    if (dragCard) moveCard(dragCard.id, colId);
    setDragCard(null);
    setDragOver(null);
  };
      ...
      {COLUMNS.map((col) => {
        ...
        return (
          <div
            ...
            style={{
              flex: "1 0 240px", maxWidth: 320, minWidth: 240,
              background: dragOver === col.id ? "#1a1a2e" : "#0f0f1a",
              border: `1px solid ${dragOver === col.id ? col.color + "80" : "#1e1e3a"}`,
            }}
          >
            {/* Column header */}
            <div className="px-3.5 pt-3.5 pb-2.5 border-b border-surface-border-light flex items-center justify-between">
              <div className="flex items-center gap-2">
                {isIdea && cards.length > 0 && (
                  <input type="checkbox" className="checkbox-accent"
                    checked={cards.every((c) => c._selected)}
                    onChange={() => selectAll(col.id, cards)} />
                )}
            ...
            <div className="p-2 flex-1 overflow-y-auto flex flex-col gap-2">
              {cards.map((card) => (
                <div
                  key={card.id}
                  className="card-hover animate-slide-up rounded-xl p-3 cursor-pointer"
                  draggable
                  onDragStart={(e) => onDragStart(e, card)}
                  style={{
                    background: card._selected ? "#1e1e3a" : "#161625",
                    border: `1px solid ${card._selected ? "#6366f1" : "#2a2a4a"}`,
                    opacity: dragCard?.id === card.id ? 0.4 : 1,
                  }}
                >
                  <div className="flex gap-2 items-start">
                    {isIdea && (
                      <input type="checkbox" className="checkbox-accent mt-0.5 flex-shrink-0"
                        checked={!!card._selected}
                        onChange={(e) => { e.stopPropagation(); toggleSelect(card.id); }} />
                    )}
                    <div className="flex-1 min-w-0" onClick={() => onCardClick(card.id)}>
                      ...
                      {card.description && (
                        <div className="text-[11px] text-[#94a3b8] leading-relaxed mb-2 line-clamp-2">{card.description}</div>
                      )}
                      ...
                      {card.comments.length > 0 && (
                        <div className="mt-2 text-[10px] text-[#64748b]">💬 {card.comments.length}</div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {cards.length === 0 && (
                <div className="text-center py-6 text-[#334155] text-xs italic">Drag cards here</div>
              )}
            </div>
          </div>
        );
